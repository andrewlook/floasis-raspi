...
def cosine(x, y, cnt, **kwargs):
    ...
    scale_0 = kwargs.get('scale_0', DEFAULT_VAL) * 0.2 + 0.2
    scale_1 = kwargs.get('scale_1', DEFAULT_VAL) * 0.2 + 0.2
...
def circle(x, y, cnt, **kwargs):
    ...
    rad = np.sqrt(((1.0 * (scale_0 - x)) ** 2) + ((1.0 * (scale_1 - y)) ** 2))
    t = (speed_coef * cnt)
    retval = (
